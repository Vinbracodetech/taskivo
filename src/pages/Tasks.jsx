...
export default function Tasks({ session, navigate }) {
  ...
  // --- HARDENED FRONTEND REWARD LOGIC ---
  useEffect(() => {
    if (!user) return;

    window.onAdRewardSuccess = async () => {
      // 1. Force the 5 points directly on the frontend
      ...
      try {
        ...
        setToastMessage(`+${rewardAmount} Yield Secured! Ad completion verified.`);
  ...
  async function fetchMarketplace(isSilent = false) {
    try {
      ...
      // CPA Grip feed completely removed
      ...
      <div style={S.page}>
        <div style={S.headerWrap}>
          ...
          <button 
            onClick={() => {
              if (typeof window !== 'undefined' && window.ReactNativeWebView) {
                // Interstitial trigger on My Hub button
        ...
        >
          ...
          <div>
            ...
            <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: 24, fontWeight: 800, color: 'var(--ink)', margin: '0 0 8px 0' }}>Offerwalls & Surveys</h2>
              ...
              return (
                <div key={task.id} style={{ ...S.taskCard(isPremium, isInternalStyle), opacity: isLocked ? 0.6 : 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 24 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: isInternalStyle ? 'var(--lime-dim)' : 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, overflow: 'hidden' }}>
                        ...
                        onClick={() => {
                          if (typeof window !== 'undefined' && window.ReactNativeWebView) {
                            if (task.is_native_ad) {
                              ...
                              // Interstitial triggered for all standard tasks
                              window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'SHOW_INTERSTITIAL' }));
