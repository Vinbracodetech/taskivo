...
export default function Tasks({ session, navigate }) {
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    if (activeCategory === 'Premium & Growth') return task.is_cpa || task.is_native_ad || ['ugc', 'qa_testing', 'growth'].includes(task.platform);
    return true;
  });
  ...
  const S = {
    ...
    taskCard: (isPremium, isInternalStyle, isCpa) => ({ 
      background: 'var(--surface-card)', 
      border: '1px solid rgba(255,255,255,0.03)', 
      borderLeft: `4px solid ${isCpa ? '#ef4444' : isPremium ? '#D4AF37' : isInternalStyle ? '#A8FF3E' : 'rgba(255,255,255,0.1)'}`,
      borderRadius: 16, 
      padding: '24px', 
      display: 'flex', 
    ...
    btnActive: (isPremium, isInternalStyle, isCpa) => ({ background: isCpa ? '#ef4444' : isPremium ? 'var(--gold)' : isInternalStyle ? 'var(--lime-dim)' : 'var(--lime)', color: (isInternalStyle && !isCpa && !isPremium) ? 'var(--lime)' : (isCpa ? '#fff' : '#000'), border: isInternalStyle ? '1px solid var(--lime)' : 'none', padding: '10px 24px', borderRadius: 100, fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: "'Inter', sans-serif", textTransform: 'uppercase', letterSpacing: '0.5px' }),
      ...
      <div style={S.page}>
        ...
        >
          ...
          <div>
            ...
            <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: 24, fontWeight: 800, color: 'var(--ink)', margin: '0 0 8px 0' }}>Offerwalls & CPA Surveys</h2>
        ...
        {filteredTasks.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
            {displayedTasks.map(task => {
              ...
              return (
                <div key={task.id} style={{ ...S.taskCard(isPremium, isInternalStyle, isCpa), opacity: isLocked ? 0.6 : 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 24 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: isCpa ? 'rgba(239,68,68,0.1)' : isInternalStyle ? 'var(--lime-dim)' : 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, overflow: 'hidden' }}>
                      {task.image ? <img src={task.image} alt="app" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : icon}
                    </div>
                    <div style={{ flex: 1, paddingRight: (isPremium || isCpa) ? 70 : 0 }}>
                      <div style={{ fontSize: 10, fontWeight: 800, color: isCpa ? '#ef4444' : isInternalStyle ? 'var(--lime)' : 'var(--slate)', textTransform: 'uppercase', marginBottom: 6, letterSpacing: '0.5px' }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--slate)', display: 'flex', alignItems: 'center', gap: 6 }}>
                         <span style={{ display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: isCpa ? '#ef4444' : isPremium ? '#D4AF37' : 'var(--lime)' }} />
                         {isCpa ? 'S2S Automated Verification' : task.is_native_ad ? 'Native App Exclusive' : (isPremium ? 'Manual Verification' : (task.watch_duration ? `${task.watch_duration}s Enforced Dwell` : 'Automated Verification'))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto', paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 20, fontWeight: 800, color: isLocked ? 'var(--slate)' : isCpa ? '#ef4444' : isInternalStyle ? 'var(--lime)' : 'var(--ink)', fontFamily: "'Inter', sans-serif" }}>+{task.reward_points} <span style={{ fontSize: 12, color: 'var(--slate)', fontWeight: 500 }}>PTS</span></div>
                    </div>
