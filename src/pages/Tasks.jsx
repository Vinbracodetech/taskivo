...
export default function Tasks({ session, navigate }) {
  ...
  const S = {
    ...
    // Unified, sleek dashboard panel for quotas
    quotaPanel: { background: 'var(--surface-card)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: 24, display: 'flex', flexWrap: 'wrap', gap: 24, marginBottom: 40, boxShadow: '0 8px 32px rgba(0,0,0,0.2)' },
    quotaItem: { flex: '1 1 120px', display: 'flex', flexDirection: 'column', gap: 8 },
    
    // Modernized Task Cards (No heavy outlines, sleek left-border accent)
  ...
  return (
    <div style={S.page}>
      <div style={S.headerWrap}>
        ...
        <button onClick={() => navigate('user-dashboard')} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--ink)', borderRadius: 100, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'background 0.2s' }}>My Hub →</button>
      </div>

      {/* 🔥 THE NEW UNIFIED QUOTA PANEL 🔥 */}
      ...
      {tasks.length === 0 ? (
         <div style={{ background: 'var(--surface-card)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 24, padding: 80, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
