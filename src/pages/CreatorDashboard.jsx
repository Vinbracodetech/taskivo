...
export default function CreatorDashboard({ user, navigate, showToast }) {
  ...
  async function fetchCreatorData() {
    try {
      setLoading(true);
      const { data: tasks, error: taskError } = await supabase.from('tasks').select('*').eq('creator_id', user.id).order('created_at', { ascending: false });
      if (taskError) throw taskError;

      let slots = 0, verified = 0;
      if (tasks) {
        tasks.forEach(function(t) {
          slots += (t.target_views || 0); verified += (t.current_views || 0);
        });
      }

      setStats({ totalCampaigns: tasks ? tasks.length : 0, activeSlots: slots, verifiedEngagements: verified });
      setRecentCampaigns(tasks ? tasks.slice(0, 3) : []);

      const { data: pending, error: pendingError } = await supabase.from('completions').select('id, proof_text, proof_url, created_at, tasks!inner(title, reward_points, creator_id)').eq('tasks.creator_id', user.id).eq('status', 'pending').order('created_at', { ascending: false }).limit(10);
      if (!pendingError && pending) setPendingReviews(pending);
  ...
  async function fetchMessages() {
    try {
      const { data, error } = await supabase.from('user_messages').select('*').order('created_at', { ascending: false }).limit(10);
  ...
  async function handleSubmitTicket() {
    ...
    try {
      const { error } = await supabase.from('support_tickets').insert({ user_id: user.id, category: ticketForm.category, message: ticketForm.message });
  ...
  async function handleQADecision(completionId, decision) {
    ...
    try {
      const { error } = await supabase.from('completions').update({ status: decision }).eq('id', completionId);
      if (error) throw error;
...
\x3C/script>`;
  ...
  const S = {
    pageWrapper: { minHeight: '100vh', backgroundColor: '#0a0a0c', backgroundImage: `radial-gradient(circle at 15% 50px, rgba(212, 175, 55, 0.08) 0%, transparent 40%), radial-gradient(circle at 85% 30%, rgba(168, 255, 62, 0.04) 0%, transparent 50%), url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 0h40v40H0V0zm20 20h20v20H20V20zM0 20h20v20H0V20z' fill='none' stroke='%23ffffff' stroke-width='1' stroke-opacity='0.02'/%3E%3C/svg%3E")`, backgroundSize: '100% 100%, 100% 100%, 40px 40px', backgroundAttachment: 'fixed' },
    page: { padding: '40px 5%', maxWidth: 1200, margin: '0 auto', fontFamily: "'DM Sans', sans-serif", position: 'relative' },
    glassCard: { background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.06)', borderTop: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: 20, padding: 32, boxShadow: '0 16px 40px rgba(0,0,0,0.2)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' },
      ...
      <div style={S.page}>
        ...
        <div style={{ marginBottom: 56, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 24, borderBottom: '1px solid rgba(255, 255, 255, 0.1)', paddingBottom: 32 }}>
          <div>
            ...
            <h1 style={S.header}>Business Command</h1>
        ...
        {/* 🔥 ALWAYS-VISIBLE INBOX WIDGET FOR CREATORS 🔥 */}
        <div 
          onClick={() => setShowInboxModal(true)}
          style={{ ...S.glassCard, position: 'relative', zIndex: 1, marginBottom: 56, cursor: 'pointer', transition: 'all 0.2s', border: activeMessages.length > 0 ? '1px solid rgba(212, 175, 55, 0.3)' : '1px solid rgba(255, 255, 255, 0.06)', background: activeMessages.length > 0 ? 'rgba(212, 175, 55, 0.03)' : 'rgba(255, 255, 255, 0.02)' }}
          onMouseOver={(e) => e.currentTarget.style.transform = 'translateY(-2px)'}
          onMouseOut={(e) => e.currentTarget.style.transform = 'translateY(0)'}
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ background: activeMessages.length > 0 ? '#D4AF37' : 'rgba(255,255,255,0.05)', color: activeMessages.length > 0 ? '#000' : 'rgba(255,255,255,0.5)', width: 48, height: 48, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, transition: 'all 0.3s' }}>
            ...
            <div style={{ background: activeMessages.length > 0 ? 'rgba(212, 175, 55, 0.1)' : 'rgba(255,255,255,0.05)', color: activeMessages.length > 0 ? '#D4AF37' : 'rgba(255,255,255,0.5)', padding: '6px 12px', borderRadius: 100, fontSize: 12, fontWeight: 800, fontFamily: "'Inter', sans-serif" }}>
              ...
              {pendingReviews.map(review => (
                <div key={review.id} style={S.qaCard}>
                  ...
                  <div style={{ display: 'flex', gap: 12, marginTop: 'auto' }}>
                    <button onClick={() => handleQADecision(review.id, 'rejected')} disabled={processingId === review.id} style={{ ...S.btnReject, flex: 1, opacity: processingId === review.id ? 0.5 : 1 }}>Reject (Refund Escrow)</button>
                    <button onClick={() => handleQADecision(review.id, 'approved')} disabled={processingId === review.id} style={{ ...S.btnApprove, flex: 1, opacity: processingId === review.id ? 0.5 : 1 }}>Approve & Release</button>
                  </div>
                </div>
              ))}
            </div>
                ...
                {recentCampaigns.map(function(campaign, index) {
                  ...
                  return (
                    <div key={campaign.id} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 20, padding: '24px', borderBottom: isLast ? 'none' : '1px solid rgba(255, 255, 255, 0.06)', alignItems: 'center', transition: 'background 0.2s', cursor: 'pointer' }} onMouseOver={(e) => e.currentTarget.style.background = 'rgba(255, 255, 255, 0.03)'} onMouseOut={(e) => e.currentTarget.style.background = 'transparent'}>
                      ...
                      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>{campaign.platform.replace('_', ' ')}</div>
                      <div>
                      ...
                      <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                        <span style={{ background: campaign.status === 'active' ? 'rgba(212, 175, 55, 0.1)' : 'rgba(255, 255, 255, 0.05)', color: campaign.status === 'active' ? '#D4AF37' : 'rgba(255,255,255,0.5)', border: `1px solid ${campaign.status === 'active' ? 'rgba(212, 175, 55, 0.3)' : 'rgba(255, 255, 255, 0.1)'}`, fontSize: 10, fontWeight: 800, padding: '6px 12px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '1.5px', display: 'inline-block' }}>{campaign.status}</span>
                        {campaign.platform === 'blog' && (
                          <button onClick={(e) => { e.stopPropagation(); setScriptModal({ isOpen: true, task: campaign }); }} style={{ background: 'rgba(212, 175, 55, 0.1)', color: '#D4AF37', border: '1px solid rgba(212, 175, 55, 0.3)', padding: '6px 12px', borderRadius: 4, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', cursor: 'pointer', transition: 'all 0.2s' }}>
                            {"</>"} Get Node Script
                          </button>
                        )}
                      </div>
        ...
        <div style={{ ...S.glassCard, position: 'relative', zIndex: 1 }}>
          ...
          {tickets.length === 0 ? (
            <div style={{ padding: '32px 0', textAlign: 'center', color: 'rgba(255,255,255,0.5)', fontSize: 14, borderTop: '1px solid rgba(255,255,255,0.05)' }}>No open tickets. You are all caught up!</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {tickets.map(ticket => (
                <div key={ticket.id} style={S.ticketCard}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#ffffff', textTransform: 'uppercase', letterSpacing: '1px' }}>{ticket.category}</div>
                    <div style={{ fontSize: 10, fontWeight: 800, padding: '4px 8px', borderRadius: 6, letterSpacing: '0.5px', textTransform: 'uppercase', ...(ticket.status === 'resolved' ? S.statusResolved : S.statusOpen) }}>{ticket.status}</div>
                  </div>
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 16 }}>Submitted: {new Date(ticket.created_at).toLocaleDateString()}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={S.modalOverlay}>
            <div style={S.modalCard}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                ...
                <button onClick={handleSubmitTicket} disabled={submittingTicket} style={{ ...S.btnPrimary, flex: 1, opacity: submittingTicket ? 0.5 : 1 }}>{submittingTicket ? 'Submitting...' : 'Send to Admin'}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={S.modalOverlay}>
            <div style={{...S.modalCard, maxWidth: 600, padding: 0, overflow: 'hidden'}}>
              <div style={{ padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.2)' }}>
                <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: 20, fontWeight: 800, color: '#ffffff', margin: 0 }}>Notification Center</h2>
                <button onClick={() => setShowInboxModal(false)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 24, cursor: 'pointer' }}>×</button>
              </div>
              ...
              <div style={{ padding: 32, maxHeight: '60vh', overflowY: 'auto' }}>
                {activeMessages.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '40px 0', color: 'rgba(255,255,255,0.5)' }}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 700, color: '#ffffff', marginBottom: 8 }}>You're all caught up!</div>
                    <div style={{ fontSize: 14 }}>No new alerts at this time.</div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {activeMessages.map(msg => (
                      <div key={msg.id} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, padding: 20, position: 'relative' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                          <div style={{ fontSize: 15, fontWeight: 800, color: '#ffffff', fontFamily: "'Inter', sans-serif", paddingRight: 40 }}>{msg.title}</div>
                          <div style={{ fontSize: 11, color: '#D4AF37', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                            {new Date(msg.created_at).toLocaleDateString()}
                          </div>
                        </div>
                        <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, lineHeight: 1.6, whiteSpace: 'pre-wrap', marginBottom: 16 }}>
                          {msg.message}
                        </div>
                        <button 
                          onClick={() => dismissMessage(msg.id)}
                          style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', padding: '6px 16px', borderRadius: 100, fontSize: 11, fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', transition: 'all 0.2s' }}
                          ...
                          onMouseOut={(e) => { e.currentTarget.style.color = 'rgba(255,255,255,0.6)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; e.currentTarget.style.background = 'transparent'; }}
                        >
                          ✕ Dismiss
                        </button>
                      </div>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: '#0a0a0c', border: '1px solid #D4AF37', borderRadius: 20, width: '100%', maxWidth: 700, overflow: 'hidden', boxShadow: '0 24px 50px rgba(0,0,0,0.5)' }}>
              ...
              <div style={{ padding: 32 }}>
                <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.7)', lineHeight: 1.6, marginTop: 0, marginBottom: 24 }}>Paste this snippet directly into the HTML of your target article to enable zero-bot verification. This script is permanently bound to Task ID: <span style={{ fontFamily: 'monospace', color: '#D4AF37' }}>{scriptModal.task.id.substring(0,8)}...</span></p>
