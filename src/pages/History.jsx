...
export default function History({ session, navigate }) {
    ...
    try {
      ...
      if (activeTab === 'engagements') {
        if (isCreator) {
          ...
          const [compRes, blogRes, spinRes, bonusRes] = await Promise.allSettled([
          ...
          // 1. Inject Standard & Premium Campaigns
          ...
          // 4. 🔥 Inject Telegram Bonus 🔥
          if (bonusRes.status === 'fulfilled' && bonusRes.value.data) {
            mergedTimeline.push(...bonusRes.value.data.map(d => ({
              id: d.id,
              created_at: d.created_at,
              ...
              if (activeTab === 'engagements') {
                if (isCreator) {
                  ...
                  if (item._recordType === 'official_blog') {
                    ...
                    tagColor = '#D4AF37';
                  } else if (item._recordType === 'telegram_bonus') {
                    // 🔥 UI FOR TELEGRAM BONUS 🔥
                    typeColor = '#0088cc'; 
                    bgColor = 'rgba(0, 136, 204, 0.1)';
                    tagText = 'Community Bonus';
                    tagColor = '#0088cc';
                  }
