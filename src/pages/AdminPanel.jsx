          ...
          return (
            <div key={task.id} style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 16, padding: 24, display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center', justifyContent: 'space-between', boxShadow: C.shadow }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {/* Delete Button Only Shows for Rejected or Completed Tasks */}
                {isDeletable && (
    ...
    <div style={{ padding: '40px 5%', maxWidth: 1000, margin: '0 auto', fontFamily: "'DM Sans', sans-serif", paddingBottom: 120 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {users.map(function(u) {
          return (
            <div key={u.id} style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: C.textMain }}>{u.email}</div>
