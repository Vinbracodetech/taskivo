...
export default function Auth({ authMode, setAuthMode, navigate, loadProfile }) {
  ...
  var gridStyle = {
    position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", backgroundSize: "48px 48px",
  ...
  var leftStyle = {
    flex: "0 0 45%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 56px", position: "relative", zIndex: 1,
    opacity: mounted ? 1 : 0, transform: mounted ? "translateX(0)" : "translateX(-24px)", transition: "opacity 0.7s ease, transform 0.7s ease",
  };

  var rightStyle = {
    flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 48px", position: "relative", zIndex: 1,
    opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.7s ease 0.15s, transform 0.7s ease 0.15s",
  };
  ...
  var googleBtnStyle = {
    width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "12px 20px", background: "rgba(255,255,255,0.07)",
    border: "1px solid rgba(255,255,255,0.12)", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", cursor: "pointer", transition: "all 0.2s", marginBottom: 20,
  };
  ...
  var roleTabStyle = function (active) {
    return { flex: 1, padding: "10px 0", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 13, background: active ? "#A8FF3E" : "transparent", color: active ? "#0D0D14" : "rgba(255,255,255,0.4)", transition: "all 0.2s" };
  };
      ...
      <div style={{ ...pageStyle, alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ ...cardStyle, maxWidth: 400, textAlign: "center", zIndex: 1, opacity: mounted ? 1 : 0, transition: "opacity 0.5s ease" }}>
          ...
          <div style={{ fontSize: 14, color: "rgba(255,255,255,0.4)", lineHeight: 1.7, marginBottom: 28 }}>
            We sent a confirmation link to <span style={{ color: "#A8FF3E", fontWeight: 700 }}>{form.email}</span>. Click it to activate your account.
          </div>
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0D0D14", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 20px", position: "relative", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ zIndex: 1, marginBottom: 32, textAlign: "center" }}>
          <div onClick={function() { if(navigate) navigate(''); }} style={{ ...logoTextStyle, fontSize: 22, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, cursor: 'pointer' }}>
            ⚡ Taskivo <span style={logoBadgeStyle}>BETA</span>
          </div>
        </div>
        ...
        <div style={{ ...cardStyle, zIndex: 1, width: "100%" }}>
          ...
          <button style={googleBtnStyle} onClick={handleGoogle} disabled={loading}>
            <img src="https://www.google.com/favicon.ico" width={16} height={16} alt="G" /> Continue with Google
          </button>

          <div style={dividerStyle}>
            <div style={dividerLineStyle} /><span style={dividerTextStyle}>OR</span><div style={dividerLineStyle} />
          </div>
          ...
          <div style={switchStyle}>
            {mode === "login" ? (
              <>Don't have an account? <span style={switchLinkStyle} onClick={function () { setMode("register"); setError(""); }}>Sign up free</span></>
            ) : (
              <>Already have an account? <span style={switchLinkStyle} onClick={function () { setMode("login"); setError(""); }}>Log in</span></>
            )}
          </div>
        </div>
      </div>
          ...
          {featuresData.map(function (f) {
            return (
              <div key={f.label} style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(168,255,62,0.1)", border: "1px solid rgba(168,255,62,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
                  {f.icon}
                </div>
                <div>
