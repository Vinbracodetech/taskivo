...
export default function DailyRewardWidget({ session }) {
  ...
  // ── BROKEN DOWN STYLES TO PREVENT COPY-PASTE BUGS ──
  const cardStyle = {
  ...
  const btnStyle = {
    background: canClaim ? 'var(--lime)' : 'var(--surface)',
    color: canClaim ? '#000' : 'var(--slate)',
    border: 'none',
    padding: 12,
    borderRadius: 8,
    fontWeight: 800,
  ...
  return (
    <div style={cardStyle}>
      ...
      <h3 style={{ color: 'var(--ink)', margin: '8px 0' }}>
        Daily Check-in
      </h3>
      ...
      <button onClick={handleClaim} disabled={!canClaim} style={btnStyle}>
